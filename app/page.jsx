...
export default function Home() {
  ...
  return <>
    ...
    <section className="people-callout" id="team"><div><h2>Our Group</h2><Link className="text-link" href="/team/">Team</Link></div><div className="people-preview">{people.filter(p => p.status === 'member' && p.image).slice(0, 4).map(p => <Link href={`/people/${p.id}/`} key={p.id}><PersonPhoto person={p} loading="lazy" /><span>{p.name}</span></Link>)}</div></section>
