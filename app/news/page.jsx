...
export default function News() {
  const legacyAwards = papers.filter(
    (p) => p.award && !awards.some((a) => a.publicationIds?.includes(p.id)),
  );
  return (
    <>
          ...
          {legacyAwards.map((p) => (
            <article className="news-item" key={p.id}>
              ...
              <div>
                ...
                <h3>
                  <Link href={`/publications/${p.id}/`}>{p.title}</Link>
                </h3>
