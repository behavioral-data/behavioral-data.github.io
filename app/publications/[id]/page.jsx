...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    other: p ? { citation_title: p.title, citation_publication_date: String(p.year) } : {},
  };
}
    ...
    <>
      <header className="page-heading">
        ...
        <div className="hero-links">
          {p.pdf && (
            <a className="button" href={p.pdf}>
              PDF
            </a>
          )}
          {p.arxivId && <a href={`https://arxiv.org/abs/${p.arxivId}`}>arXiv</a>}
          {safeUrl(doi) && <a href={safeUrl(doi)}>DOI</a>}
          {safeUrl(p.dataset) && <a href={safeUrl(p.dataset)}>Dataset</a>}
          {safeUrl(p.code) && <a href={safeUrl(p.code)}>Code</a>}
          {safeUrl(p.url) && <a href={safeUrl(p.url)}>Website</a>}
        </div>
      </header>
      {p.image && (
        <img className="detail-image" src={p.image} alt={`Research illustration for ${p.title}`} />
      )}
      {p.description && (
      ...
      {updates.length > 0 && (
        <section className="section">
          <h2>News</h2>
          <NewsList news={updates} />
        </section>
      )}
      <AwardList awards={awardsFor(awards, 'publicationIds', p.id)} />
      <ProjectList
        projects={projects.filter((project) => project.publicationIds?.includes(p.id))}
      />
      <section className="section">
        <h2>BibTeX</h2>
        <pre className="citation">{bibtex(p)}</pre>
      </section>
    </>
  );
}
