...
5. Review venue, year, publication state, ordered authors, URL, and whether the work belongs to the lab. Include qualifying preprints with both `status` and `type` set to `preprint` so the site labels them; prefer a final published version when one exists and do not create both versions.
