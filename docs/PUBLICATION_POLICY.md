...
Start with `maintenance/recent-review.md`; the full queue is in `maintenance/batch.md` and `maintenance/review.json`. Scholar findings and source links are recorded in `_planning/SCHOLAR_LATEST_REVIEW.md`. The existing 47 approved publications were unchanged by discovery; the first approved batch adds 12 records, including four visibly labeled preprints. No live-sync checkpoint was written, and discovery and scheduled workflows remain disabled during this pilot. Complete historical coverage and membership intervals remain review tasks.
