...
The registry fetches the nine current members’ primary records. Rongwu was added on September 10 with his website-linked Scholar identity and two ACL Anthology paper anchors; unanchored work still needs identity review. It also recognizes anchored identities for past members, including Ashish Sharma, Galen Weld, Joy He-Yueya, Ken Gu, Mike Merrill, Xinyi Zhou, Yasaman Sefidgar, Mickel Liu, and Vinayak Gupta. These entries use `discover: false`: they match coauthors in retrieved papers without fetching their entire subsequent careers. Additional past members and split IDs still need verification. Coverage is preliminary, so papers with too few matches remain visible in the full durable queue, but only candidates containing Tim can enter the focused review set.
...
The queue retains 390 observations grouped into 321 canonical candidates: 18 accepted, 302 pending and one rejected, plus one accepted manual supplement. Pending screening labels are 38 membership-review and 264 does-not-meet-rule. The Cognitive Reframing preprint is now grouped under its accepted ACL record. Vinayak Gupta’s identity is anchored to the forecasting paper and its preprint, with career-wide discovery disabled. Acceptance audits used the maintainer-approved year-level membership evidence. Screening itself was subsequently updated to consume that evidence in the September 10 attribution pass. Full evidence and remaining work are in `_planning/PUBLICATION_BACKFILL_REVIEW.md`.
...
## September 10 readiness check

All 66 archive records now carry explicit profile author links, and the screening pipeline consumes reviewed inclusive membership years. The isolated live pilot covered all nine current discovery identities, retrieved 402 unique works, and produced an identical queue on both incremental and cached repeat runs. The website bibliography, production review queue, and all prior decisions stayed unchanged. See [maintenance pilot](../_planning/MAINTENANCE_PILOT.md) for provider access, request counts, identity limitations, and activation requirements.
