...
| `awards.json` | `id`, `title`, `organization`, `date`, `sourceUrl`, at least one recipient or paper | `personIds`, `publicationIds`, `recipientNames` for alumni without profiles, `recipientLabel` for team attribution, `kind`, and `dateLabel` for a verified year range; one record supplies award labels and related profile, publication, and news views. |
