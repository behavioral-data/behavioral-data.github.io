...
| `publications.json` | `id`, `title`, `authors`, integer `year` | Ordered `authorNames`, explicit `personIds`, `venue`, optional `venueShort`, DOI, `openalexId`, `arxivId`, `status`, `type`, `topics`, `description` (Markdown), `highlight`, local/external `pdf`, local `image`, `url`, `code`, `dataset`, `reviewedOn`, preserved `legacy`. |
