...
export function filterPapers(papers, { query = '', year = '', author = '', person = null, topic = '', venue = '', type = '', awardsOnly = false } = {}) {
  ...
  return papers.filter(p => (!year || String(p.year) === year)
    && (!person ? (!author || matchesPerson(p, author)) : (Array.isArray(p.personIds) ? p.personIds.includes(person.id) : [person.name, ...(person.aliases || [])].some(name => matchesPerson(p, name)))) && (!topic || p.topics?.includes(topic)) && (!venue || p.venue === venue) && (!type || p.type === type) && (!awardsOnly || !!p.award)
...
export function bibtex(paper) {
  ...
  const clean = value => String(value || '').replace(/\s+/g, ' ').trim().replace(/[\\&%$#_~^]/g, character => replacements[character]);
