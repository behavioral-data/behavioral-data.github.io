...
export function groupAwardsByRecipient(awards, people) {
  ...
  for (const award of awards) {
    const recipients = [
      ...
      ...(award.recipientNames || []).map((name) => ({ id: `name:${name}`, name })),
    ];
    for (const recipient of recipients) {
      if (!groups.has(recipient.id)) groups.set(recipient.id, { ...recipient, awards: [] });
      const group = groups.get(recipient.id);
      if (!group.awards.some((a) => a.id === award.id)) group.awards.push(award);
    }
  }
